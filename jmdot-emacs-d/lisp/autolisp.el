...
;;; Created: 2000-01-12
;;; Time-stamp: <Thu Nov 01 14:51:53 JST 2018>
...
(defcustom autolisp-frame-width nil
...
# [ ] , are valid symbol constituents (though they should not be used)"
  ...
  (if (not autolisp-mode-syntax-table)
    (progn (setq autolisp-mode-syntax-table
...
; (setq mode-line-process '("" lisp-mode-line-process))
  ...
  ;; XEmacs defaults:
  ;;  (nth 3 (get 'lisp-mode 'font-lock-defaults))
  ;;  =>  ((?: . "w") (?- . "w") (?* . "w") (?+ . "w") (?. . "w") (?< . "w")
  ;;	(?> . "w") (?= . "w") (?! . "w") (?? . "w") (?$ . "w") (?% . "w")
  ;;	(?_ . "w") (?& . "w") (?~ . "w") (?^ . "w") (?/ . "w"))
  (setq font-lock-defaults
	(append
	 '((autolisp-font-lock-keywords
	   autolisp-font-lock-keywords-1 autolisp-font-lock-keywords-2)
	   nil t)
...
;;  (autolisp-font-lock-init) ;; commented out by JM
  (autolisp-menu-init)
  (autolisp-syntax-table-init)
...
; DONE: make it buffer local, fix GNU Emacs default colors
; like this:
;(add-hook 'autolisp-mode-hook 
;  '(lambda () 
